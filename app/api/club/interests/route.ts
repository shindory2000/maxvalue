...
export async function GET(request: NextRequest) {
  ...
  const { data: profiles, error } = await supabase.from("seeker_profiles").select("id,user_id,nickname,age,desired_region,desired_area,work_experience,desired_shift,start_timing,photo_1_url,photo_2_url,full_body_photo_url,bubble_raw,created_at").order("created_at", { ascending: false });
  ...
  const userIds = rows.map(profile => profile.user_id).filter(Boolean);
  const { data: users } = userIds.length ? await supabase.from("users").select("id,line_picture_url").in("id", userIds) : { data: [] };
...
export async function PATCH(request: NextRequest) {
  ...
  const access = await requireClubAccess(request, clubId);
  if (access.error) return access.error;
  const supabase = access.supabase || getSupabaseServer();
  if (!supabase) return NextResponse.json({ error: "Supabase is not configured" }, { status: 503 });
