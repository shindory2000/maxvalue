...
export async function GET(request: NextRequest) {
  ...
  const [{ data: clubs }, { data: responses }] = await Promise.all([
    clubIds.length ? supabase.from("clubs").select("id,display_name,area,logo_url").in("id", clubIds) : Promise.resolve({ data: [] }),
    offerIds.length ? supabase.from("offer_responses").select("offer_id,status,response,response_status,next_action,selected_date,response_source,line_payload,created_at").in("offer_id", offerIds).order("created_at", { ascending: false }) : Promise.resolve({ data: [] }),
  ]);
