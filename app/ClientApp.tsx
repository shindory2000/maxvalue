...
function SentOffers({ go }: { go: (s: Screen) => void }) {
  type ClubOffer = { id: string; seeker_name: string; hourly_wage: number; guarantee_period: string; comment: string; created_at: string; workflow_status: string; selected_date?: string | null; next_action?: string | null; cancel_reason?: string | null; response?: Record<string, unknown>; options?: string[]; workflow?: Record<string, unknown> };
  const { selectedClub } = useAdminCapability();
